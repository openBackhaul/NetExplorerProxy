"""Test 5: API: /v1/provide-air-interface-transmission-mode-lists-of-devices."""
...
class TestProvideAirInterfaceTransmissionModeListsOfDevices:
    """Test class for /v1/provide-air-interface-transmission-mode-lists-of-devices API."""

    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_returns_response_and_air_interface_transmission_modes_columns_exist(
        self,
    ):
        """Test that returns response and air_interface_transmission_modes columns exist."""
        test_file = __file__
        test_name = (
            "test_returns_response_and_air_interface_transmission_modes_columns_exist"
        )

        test_case = TEST_CASES["provideAirInterfaceTransmissionMode"]
    ...
    async def test_returns_400_when_mount_name_list_contains_non_string_items(self):
        ...
        response = await run_invalid_field_type_case(
            test_case, {"mount-name-list": [12345]}
        )
        assert_error_contract(response)
    ...
    async def test_returns_400_with_structured_error_body_when_known_field_has_wrong_type(
        ...
        test_case = TEST_CASES["provideAirInterfaceTransmissionMode"]
        response = await run_invalid_field_type_case(
            test_case, {"mount-name-list": {"invalid": True}}
        )
