 ...
 * @param next
 * results:
 * 200 List of MAC tables of all connected devices.
 * 500 Internal server error - internal NEP error
 * 502 Bad gateway - error caused by MATR, e.g. MATR unavailable
 */
...
/**
 * Handler for /v1/provide-mac-table-of-all-devices
 * @param req
 * @param res
 * @param next
 * results:
 * 200 List of MAC tables of all devices.
 * 500 Internal server error - internal NEP error
 * 502 Bad gateway - error caused by MATR, e.g. MATR unavailable
 */
module.exports.provideMacTableOfAllDevices = async function provideMacTableOfAllDevices (req, res, next) {
  let startTime = process.hrtime();

  try {
    const ret = await individualServices.provideMacTableOfAllDevices(req.url);
    return handleForwardedResult(startTime, req, res, ret);
  } catch (error) {
    return handleError(startTime, error, req, res);
  }
...
/**
 * Handler for /v1/provide-mac-table-of-specific-device
 * results:
 * 200 MAC table of the requested device.
 * 500 Internal server error - internal NEP error
 * 502 Bad gateway - error caused by MATR, e.g. MATR unavailable
 */
module.exports.provideMacTableOfSpecificDevice = async function provideMacTableOfSpecificDevice (req, res, next, body) {
  let startTime = process.hrtime();

  try {
    const ret = await individualServices.provideMacTableOfSpecificDevice(req.url, body);
...
/**
 * Handler for /v1/read-current-mac-table-from-device
 * @param req
 * @param res
 * @param next
 * @param body
 * results:
 * 200 request ID
 * 429 Too many requests. The maximum amount of requests that can executed in parallel or per day has been reached
 * 500 Internal server error - internal NEP error
 * 502 Bad gateway - error caused by MATR, e.g. MATR unavailable
 */
module.exports.readCurrentMacTableFromDevice = async function readCurrentMacTableFromDevice (req, res, next, body) {
  let startTime = process.hrtime();

  try {
    const ret = await individualServices.readCurrentMacTableFromDevice(req.url, body);
...
/**
 * Handler for /v1/receive-current-mac-table-of-device
 * @param req
 * @param res
 * @param next
 * @param body
 ...
 * 502 Bad gateway - error caused by MATR, e.g. MATR unavailable
 */
module.exports.receiveCurrentMacTableOfDevice = async function receiveCurrentMacTableOfDevice (req, res, next, body) {
  let startTime = process.hrtime();

  try {
    const ret = await individualServices.receiveCurrentMacTableOfDevice(req.url, body);
...
/**
 * Handler for /v1/provide-actual-equipment-information-of-devices
 * @param req
 * @param res
 * @param next
 * @param body
...
/**
 * Handler for /v1/provide-wire-interface-general-information-of-devices
 * @param req
 * @param res
 * @param next
 * @param body
...
/**
 * Handler for /v1/provide-air-interface-general-information-of-devices
 * @param req
 * @param res
 * @param next
 * @param body
...
/**
 * Handler for /v1/provide-air-interface-transmission-mode-lists-of-devices
 * @param req
 * @param res
 * @param next
 * @param body
