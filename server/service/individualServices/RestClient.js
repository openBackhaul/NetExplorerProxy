    ...
    return await axios.post(targetUrl, payload, {
        ...
        .catch(e => {
            ...
            executionAndTraceService.recordServiceRequestFromClient(
                ...
                requestHeader.originator,
                operationName,
                e.response && e.response.status? e.response.status: 502,
                payload,
                e,
                targetUrl);
...
exports.startPostDataRequest = async function (targetUrl, payload, operationName, operationKey) {
    ...
    return await axios.post(targetUrl, payload, {
        ...
        .then((response) => {
            ...
            return {code: response.status, message: response.data, headers: response.headers};
        })
        .catch(e => {
            logger.error(e, "error during " + operationName);
            let response = e.response ?? {status: 502, data: e};

            if (response.status === 500) {
                response.status = 502; // Bad gateway - error caused by MATR/MWDI
            }
